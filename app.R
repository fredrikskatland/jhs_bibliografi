...
ui <- fluidPage(
   ...
   sidebarLayout(
      sidebarPanel(
        ...
        textInput("tittel", "Figurtittel", "Relativ distanse til referanser")
        
        # Velg visuelle atributter
        # selectInput("xakse","X-akse",""),
        # selectInput("yakse", "Y-akse", ""),
        # selectInput("farge","Farge", ""),
...
server <- function(input, output,session) {
  observe({
    ...
    reactive_tittel <- reactive({
      return(input$tittel)
    })
    
    # Input og reactive funksjoner for å velge visuelle atributter
    ...
    #     return(NULL)
    #   
    #   ref_comp <- read.csv(inFile$datapath, header = input$header, sep = ";")
    #   kolonnenavn <- colnames(ref_comp) 
    # })
    # 
    # observeEvent(input$file1,{
    #   updateSelectInput(session, "xakse", choices = reactive_colnames())  
    # })
    # 
    # observeEvent(input$file1,{
    #   updateSelectInput(session, "yakse", choices = reactive_colnames())  
    # })
    # 
    # observeEvent(input$file1,{
    #   updateSelectInput(session, "farge", choices = reactive_colnames())  
    # })
    # 
    # observeEvent(input$file1,{
    #   updateSelectInput(session, "symbol", choices = reactive_colnames())  
    # })
    ...
    output$plot <- renderPlotly({
      ...
      plot_ly(data = ref.comp, x = ~X, y = ~Y, 
        ...
        layout(title = reactive_tittel()[1])
